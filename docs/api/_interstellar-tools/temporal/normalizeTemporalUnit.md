...
Defined in:
[normalize-temporal-unit.ts:127](https://github.com/phun-ky/interstellar-tools/blob/ed1198b9c20c47a67c6b06f4f3e5a2b0e8c00efe/packages/temporal/src/normalize-temporal-unit.ts#L127)
...
- Accepts both **canonical symbols** (e.g., `"s"`, `"ms"`, `"μs"`, `"min"`,
  ...
  using [NORMALIZE_UNIT](NORMALIZE_UNIT.md) as the single source of truth.
...
- **Durations only** (no timestamp or "ago" semantics). Geology-style tokens
  ...
  `"Myr"`/`"Gyr"`.
- **Case-sensitive** exact match against [NORMALIZE_UNIT](NORMALIZE_UNIT.md).
  For example, `"Seconds"` (capital S) will fail; use `"seconds"` instead, or
  pre-normalize casing upstream if desired.
...
The canonical [TemporalUnitType](../types/TemporalUnitType.md) (e.g., `"s"`,
`"ms"`, `"μs"`, `"min"`, `"h"`, `"d"`, `"yr"`, `"kyr"`, `"Myr"`, `"Gyr"`).

## Throws

[RangeError](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/RangeError)
If `u` does not match any known key in [NORMALIZE_UNIT](NORMALIZE_UNIT.md).
...
- [NORMALIZE_UNIT](NORMALIZE_UNIT.md) for the complete alias→canonical mapping.
