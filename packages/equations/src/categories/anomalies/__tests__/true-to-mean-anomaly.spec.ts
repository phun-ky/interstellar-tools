...
describe('trueToMeanAnomaly', () => {
  ...
  test('Circular Orbit (e = 0)', () => {
    const result = trueToMeanAnomaly((Math.PI / 3) as Radians, 0); // e = 0 (should be identity)

    assertApproxEqual(result, wrapAngle(Math.PI / 3));
  });

  test('Zero True Anomaly (V = 0)', () => {
    const result = trueToMeanAnomaly(0 as Radians, 0.5);

    assertApproxEqual(result, 0);
  });

  test('True Anomaly at π/2 (90°)', () => {
    const result = trueToMeanAnomaly((Math.PI / 2) as Radians, 0.5);
  ...
  test('True Anomaly at π (Apoapsis)', () => {
    const result = trueToMeanAnomaly(Math.PI as Radians, 0.5);
  ...
  test('True Anomaly at 3π/2 (270°)', () => {
    const result = trueToMeanAnomaly(((3 * Math.PI) / 2) as Radians, 0.5);
  ...
  test('True Anomaly at 2π (Periapsis)', () => {
    const result = trueToMeanAnomaly((2 * Math.PI) as Radians, 0.5);
  ...
  test('Eccentricity Out of Bounds Throws Error', () => {
    assert.throws(
      () => trueToMeanAnomaly((Math.PI / 3) as Radians, -0.1),
      RangeError
    );
    assert.throws(
      () => trueToMeanAnomaly((Math.PI / 3) as Radians, 1.1),
      RangeError
    );
  });

  test('Large Eccentricity (e close to 1)', () => {
    const result = trueToMeanAnomaly((Math.PI / 4) as Radians, 0.99);
